...
float4 s_vga256(float2 uv, texture2d<float> tex, constant ShaderParams& sp) {
    ...
    // Anchor all CRT physics to the VGA Mode 13h emulation resolution (320×200).
    // Input video resolution is irrelevant — a 4K source and a 480p source must
    // produce identical phosphor structure at the same display size.
    const float emuW = 320.0, emuH = 200.0;

    // ── 1. Barrel distortion ──────────────────────────────────────────────────
    float2 w = barrel(uv, sp.p0, sp.p0 * 1.25);
    if (oob(w)) return float4(0.0, 0.0, 0.0, 1.0);

    // ── 2. Snap to VGA grid — chunky 320×200 logical pixels ──────────────────
    // Quantise the sample UV to the nearest VGA pixel centre so all CRT effects
    // operate on discrete logical pixels, not sub-pixels of the source video.
    float2 wSnap = (floor(w * float2(emuW, emuH)) + 0.5) / float2(emuW, emuH);

    // ── 3. Convergence error — per-channel lateral offset (±1 VGA pixel) ──────
    float cx = 1.0 / emuW;
    float3 base;
    base.r = tex.sample(s, wSnap + float2(-cx, 0.0)).r;
    base.g = tex.sample(s, wSnap).g;
    base.b = tex.sample(s, wSnap + float2( cx, 0.0)).b;
    float3 baseLinear = lin(base);

    // ── 4. P22 phosphor persistence — 7-tap Gaussian in VGA-pixel units ───────
    // Spread is expressed as a fraction of one VGA pixel so it scales correctly
    // regardless of source resolution.  R decays slowest, B fastest.
    float gw[7] = { 0.0625, 0.0938, 0.2188, 0.2500, 0.2188, 0.0938, 0.0625 };

    float sR = 1.5 / emuW, sG = 1.0 / emuW, sB = 0.7 / emuW;
    float sV = 1.0 / emuH;

    // Glow taps sample from the full-resolution source (w, not wSnap) so the
    // blur oversamples relative to the VGA grid — giving smooth phosphor halation
    // even though the visible pixel is hard-edged.
